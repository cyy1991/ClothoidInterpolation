...
namespace Clothoid {
  ...
  */
  ...
  //Yongyu Chen
  /**
   * @brief interpolation using clothoid between point(x0, y0) and (x1, y1)
   */
  void PointsOnClothoid(double x0, double y0, double theta0, double x1,
                        double y1, double theta1, vector<double>& xs,
                        vector<double>& ys, vector<double>& ks, int npts) {

      double k, dk, L;
      // build the clothoid
      buildClothoid(x0, y0, theta0, x1, y1, theta1, k, dk, L);
      // The calculated kappa k is the initial value of curvature   
      xs.clear();
      ys.clear();
      ks.clear();
      ...
      //interpolate the argument
      vector<double> tvec;
      double t = 0;
      double delta_t = (float)(L) / (npts - 1);
      for (int i = 0; i < npts; i++) {
          tvec.push_back(t);
          t += delta_t;
      }
      for (std::vector<double>::iterator itr = tvec.begin(); itr != tvec.end(); itr++) {
          double current_t = *itr;
          double C, S;
          GeneralizedFresnelCS( dk * pow(current_t, 2), k * current_t, theta0, C, S ) ;
          xs.push_back(x0 + current_t * C);
          ys.push_back(y0 + current_t * S);
          ks.push_back(k + current_t * dk);
       }
